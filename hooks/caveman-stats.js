...
// Lê o log de sessão ativo do Claude Code e exibe uso real de tokens
// + estimativa de economia baseada nos benchmarks do caveman.
//
// Uso direto:  node hooks/caveman-stats.js
// Via Claude:  /caveman-stats dispara isso pelo UserPromptSubmit hook.
...
// Média de economia por task dos benchmarks (sonnet-4, modo 'full').
// Apenas 'full' tem dados medidos; outros modos mostram sem estimativa.
const COMPRESSION = { 'full': 0.65 };

// Preço aproximado Anthropic por milhão de tokens de saída (USD).
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) return '🪨 caveman armed but no turns yet';
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  if (ratio !== null) {
    ...
    return `🪨 Saved ~${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session`;
  }
...
module.exports = {
  ...
  deriveSavings, parseSession, priceForModel, formatUsd, COMPRESSION,
